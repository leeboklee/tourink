...
import { canBookExperiences, canBookHotels } from "@/lib/feature-flags";
...
export async function POST(req: Request) {
  ...
  if (data.itemType === "hotel" && !canBookHotels()) {
    return NextResponse.json({ error: "Hotel booking is not available" }, { status: 503 });
  }
  if (data.itemType === "experience" && !canBookExperiences()) {
    return NextResponse.json({ error: "Experience booking is not available" }, { status: 503 });
  }
